# Outreach Templates – 3D Configurator

## Mijn gegevens (altijd invullen onderaan mail)

| Veld | Waarde |
|------|--------|
| Naam | Kenny Timmer |
| Telefoon | 061480282 |
| Website | bold700.com |

**In de templates:**
- **[Naam] in aanhef** = contactpersoon van het bedrijf. **Opzoeken** op LinkedIn of op de website (contactpagina, team, over ons). Als geen naam te vinden is: "Beste heer/mevrouw".
- **[Naam] in handtekening** = Kenny Timmer.
- **[Bedrijfsnaam]** = het bedrijf waar je naar mailt (bijv. Hartog Dakkapellen).
- **[Telefoon]** = 061480282.
- **[Website]** = bold700.com.
- Alle **[PLACEHOLDER]**-velden in de cold email moet je per ontvanger invullen (zie cold email hieronder).

---

## LinkedIn – Connectieverzoek

```
Hoi [Naam], ik werk aan digitale tools voor de dakkapel/bouwsector en 
zag dat jullie actief zijn met online marketing. Leuk om te connecten!
```

## LinkedIn Outreach Flow (3-stappen) – GEBRUIK DEZE

**Stap 1 – Na connectie (NOOIT direct pitchen)**
Stuur altijd eerst een laagdrempelig, persoonlijk bericht. Wacht op reactie of minimaal 1 dag.

```
Thanks voor de connectie [Naam]

Ik zie jullie advertenties vaak voorbij komen, ziet er goed uit 👍
```

of

```
Thanks [Naam]!

Jullie site ziet er goed uit
```

**Stap 2 – Fase checken (na 1 dag of na reactie)**
Stel een open vraag om te bepalen waar het bedrijf staat.

```
Ik was benieuwd: zijn jullie op dit moment bezig met vernieuwing 
van de website of tooling, of staat dat later op de planning?
```

**Stap 3 – Reageren op basis van hun antwoord**

**A. Ze zijn bezig met vernieuwing**
→ Niet pitchen. Respect tonen. Top of mind blijven.

```
Helder, logisch 👍

Dan ga ik je nu niet lastigvallen. Mocht je later eens willen 
vergelijken of sparren, laat het weten
```

**B. Ze zijn niet bezig / hebben niets**
→ DAN pas kort pitchen.

```
Dat hoor ik vaker

We bouwen 3D-configuratoren voor dakkapelbedrijven, volledig op maat, 
zodat klanten vooraf opties kunnen bekijken en jullie minder 
afstemming hebben

Zal ik 'm in 10 min laten zien?
```

**C. Ze hebben al een configurator / 3D tool**
→ Niet afkraken, vergelijken.

```
Interessant

Wat we vaak zien is dat bestaande configurators vooral visueel zijn, 
maar weinig kwalificeren richting offerte

Zal ik kort laten zien hoe wij dat aanpakken?
```

---

## Oude template (gebruik niet meer)

## Follow-up na geen reactie (1 week later)

```
Hoi [Naam],

Druk? Snap ik! Korte vraag: gebruik je momenteel een online 
configurator op jullie website?

Zo nee: ik heb een demo gemaakt van hoe dat eruit zou kunnen zien 
voor [Bedrijfsnaam]. 60 seconden video, zonder verplichtingen.

[Link naar Loom video]

Groet,
Kenny Timmer
```

## Cold email (verbeterde versie)

**Structuur:** Korte hook (visuals / dakopbouwen-aanbouwen) → probleem (statisch vs interactief) → oplossing (3D configurator + bullets) → CTA (video 60 sec). Tone: informeel maar professioneel, consequent je/jullie.

**Onderwerp:**
- **Meer conversie uit je website met 3D** (standaard)

**Alternatieve onderwerpregels (optioneel):**
1. Meer conversie uit je website met 3D
2. Vraagje over jullie offerteproces
3. Kort ideetje voor [Bedrijfsnaam]

**Body (placeholders invullen per ontvanger):**

```
Onderwerp: Meer conversie uit je website met 3D

Hoi [Naam],

Bespaar tijd in verkoopgesprekken en genereer beter 
voorbereide leads direct vanuit je website.

Ik zag dat jullie werken met visuals voor dakopbouwen 
en aanbouwen. Vaak blijft dit statisch: vaste beelden, 
beperkte keuzes, weinig interactie.

Ik heb een interactieve 3D configurator gebouwd waarmee:

- Klanten zelf opties verkennen in 3D
- Keuzes vooraf vastliggen voordat het gesprek start
- Verkopers meer regie hebben tijdens gesprekken
- Alle specs automatisch binnenkomen per lead

Volledig afgestemd op jullie productaanbod.

Korte video van 60 seconden:
https://youtu.be/MHg-PCMMyk4

Groet,
Kenny Timmer
061480282
bold700.com
```

**Placeholders om per mail in te vullen:**
| Placeholder | Voorbeeld / actie |
|-------------|-------------------|
| [Naam] | Opzoeken op LinkedIn of website (contact, team, over ons). |
| [Link naar video] | Link naar de 60-seconden demo-video (bijv. Loom of eigen host). |

## LinkedIn – Persoonlijk outreach (na onderzoek)

```
Hoi [Naam],

Ik zag jullie advertentie voor dakkapellen op Instagram - mooie projecten!

Ik heb een 3D-configurator gebouwd waarmee klanten zelf hun dakkapel/opbouw 
kunnen samenstellen en direct een prijs zien. Vergelijkbaar met wat Tesla 
doet voor auto's.

Zou je interesse hebben om te zien hoe dit er voor [Bedrijfsnaam] uit kan 
zien? Ik kan een demo maken met jullie branding.

Groet,
Kenny Timmer
```

## Bezwaren – korte antwoorden (voor demo-prep)

| Bezwaar | Antwoord |
|---------|----------|
| Te duur | "Wat kost een medewerker die 5 uur per week offertes maakt? Dit is minder dan €20 per dag." |
| We hebben al een website | "Precies, dit is een toevoeging. De configurator embed je gewoon op je bestaande site." |
| Onze prijzen zijn te complex | "De configurator kan alle variabelen aan - zo stel je je prijsregels in." |
| Klanten willen persoonlijk contact | "Klopt, maar 70% oriënteert eerst online. De configurator vangt die mensen op." |
| Ik moet erover nadenken | "Begrijpelijk. Wat zou je moeten weten om ja te zeggen?" |
| Werkt het op mobiel? | "Absoluut, 60% van gebruikers zit op telefoon. [Toon demo]" |
