...
**"HTTP 429 rate_limit_error" (50.000 of 30.000 input tokens per minute)?** De **prompt is te groot** – per verzoek gaan er te veel tokens mee. Oplossing: (1) **Nieuwe chat** (Telegram of webchat) met alleen je vraag (geen lange geschiedenis). (2) **Één bestand per beurt** (TAKEN-REEKS) – niet alles tegelijk laden. (3) Even **1–2 minuten wachten** + cooldown resetten: `./scripts/reset-clawdbot-cooldown.sh`. (4) **Config aanpassen** (in `~/.clawdbot/clawdbot.json`): `bootstrapMaxChars: 5000` (verkleint elk bootstrap-bestand). Gateway daarna herstarten. *(Let op: `contextPruning` wordt niet ondersteund door clawdbot en geeft Invalid input.)* (5) Bij Anthropic een hoger rate limit aanvragen als je vaak tegen de limiet aanloopt.
